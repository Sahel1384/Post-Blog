    ...
    <div class="container mt-5">
      ...
      <div class="row gy-3">
        <div class="table table-responsive" id="post-table">
            <table class="table table-bordered table-striped table-hover text-center m-sm-0">
                <tr>
                    {{-- <th>ID</th> --}}
                    <th>Title</th>
                ...
                @foreach ($posts as $post)
                    <tr>
                        ...
                        <td class="d-flex justify-content-center">
                            <a href="{{ route('post_form.edit', $post->id) }}" class="text-warning btn btn-sm"><span class="bi bi-pencil-square"></span></a>
                            <form action="{{ route('post_form.destroy', $post->id) }}" method="POST" enctype="multipart/form-data">
                              @csrf
                              @method('DELETE')
                              <button type="submit" class="btn btn-sm text-danger ms-3"><span class="bi bi-trash"></span></button>
                            </form>
                        </td>
