      ...
      @if ($merge->isNotEmpty())
        <div class="row  gy-3 d-flex justify-content-center">
          @foreach ($merge as $val)
            @if ($val instanceof App\Models\User)
              {{-- @if ($val->posts->isNotEmpty()) --}}
            ...
            @if($val instanceof App\Models\Post)
              <div class="col-md-4">
                <div class="cards">
                  <div class="img-user">
                    <div id="img-sub">
                      <img src="{{ '/storage/' . $val->user->image }}" alt="" /><div>
                        <span>{{ $val->user->name }}</span>
                        <p class="p">{{ $val->date }}</p>
                      </div>
                    </div>
                    <div id="img-icon">
                      <span class="btn btn-sm btn-close mt-1"></span>
                  ...
                  <a href="{{ route('seeComment', $val->id) }}" class="text-decoration-none text-dark">
                    <div id="card-info" class="mt-4">
                      <h4>{{ $val->title }}</h4>
                      <p>
                        {{ \Illuminate\Support\Str::limit($val->discription, 10, ' More...') }}
                      </p>
                    </div>
                    <div id="card-img">
                  ...
                  <div id="card-icon" class="mt-1">
                    <div>
                      <a href="{{ route('comment', $val->id) }}" class="text-dark"><i class="bi bi-chat {{ $val->comments()->where('user_id',Auth::id())->exists() ? 'text-primary' : "text-dark" }}"></i></a>
                      <p>{{ $val->comments_count }}</p>
                    </div>
                    <div class="mt-1">
                      <a href="{{ route('share', $val->id) }}" class="text-dark"><i class="bi bi-share {{ $val->shares()->where('user_id', Auth::id())->exists() ? 'text-primary' : 'text-dark' }}"></i></a>
                      <p>{{ $val->shares_count }}</p>
                    </div>
                    <div>
                      <a href="{{ route('views', $val->id) }}" class="text-dark"><i class="bi bi-eye {{ $val->views()->where('user_id',Auth::id())->exists() ? 'text-primary' : "text-dark" }}"></i></a>
                      <p>{{ $val->views_count }}</p>
                    </div>
                    <div>
                      <a href="{{ route('likes', $val->id) }}" class="text-dark">
                        <i class="bi bi-hand-thumbs-up {{ $val->likes()->where('user_id', Auth::id())->exists() ? 'text-primary' : "text-dark" }}"></i>
                      </a>
                      <p>{{ $val->likes_count }}</p>
                    </div>
                  </div>
